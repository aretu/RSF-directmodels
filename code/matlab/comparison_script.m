...
elseif query == 1
    ...
    for i=1:size(compare,2)
        ...
        try
            ...
            juliamodel=importdata(['..\julia-tests\results\',name,'rt.csv']);
    ...
    if true(overlaydata)
        switch data
            case 'brava'
                dataoverlayer(['..\..\fluid injection SHIVA BRAVA\data\jupyter\',data],711,parameters) %906.1??
            case 's1949'
                dataoverlayer(['..\..\data\jupyter\',data],711,parameters)
            case 's1997'
                dataoverlayer(['..\..\data\jupyter\',data],4446.7,parameters)
        end
    end
